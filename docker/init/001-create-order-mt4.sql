...
CREATE TABLE IF NOT EXISTS order_mt4 (
  ...
  PRIMARY KEY (ticket),
  KEY idx_order_mt4_time (`time`),
  KEY idx_order_mt4_open_time (open_time),
  KEY idx_order_mt4_login_time (login, `time`)
);

INSERT IGNORE INTO order_mt4 (
  ticket,
  login,
  symbol,
  cmd,
...
SELECT
  1000000 + sequence_numbers.seq AS ticket,
  ...
  TIMESTAMPADD(MINUTE, sequence_numbers.seq % 1052640, '2020-01-01 00:00:00') AS open_time,
  TIMESTAMPADD(MINUTE, (sequence_numbers.seq % 1052640) + 5 + (sequence_numbers.seq % 720), '2020-01-01 00:00:00') AS close_time,
  TIMESTAMPADD(MINUTE, sequence_numbers.seq % 1052640, '2020-01-01 00:00:00') AS `time`,
  CAST(
  ...
  CONCAT('generated seed order ', sequence_numbers.seq) AS comment
FROM (
