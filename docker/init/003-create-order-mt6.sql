...
CREATE TABLE IF NOT EXISTS order_mt6 (
  ...
  PRIMARY KEY (ticket),
  KEY idx_order_mt6_time (`time`),
  KEY idx_order_mt6_open_time (open_time),
  KEY idx_order_mt6_login_time (login, `time`)
);

INSERT IGNORE INTO order_mt6 (
  ticket,
  login,
  symbol,
  cmd,
...
SELECT
  3000000 + sequence_numbers.seq AS ticket,
  ...
  TIMESTAMPADD(MINUTE, sequence_numbers.seq % 2630880, '2020-01-01 00:00:00') AS open_time,
  TIMESTAMPADD(MINUTE, (sequence_numbers.seq % 2630880) + 5 + (sequence_numbers.seq % 720), '2020-01-01 00:00:00') AS close_time,
  TIMESTAMPADD(MINUTE, sequence_numbers.seq % 2630880, '2020-01-01 00:00:00') AS `time`,
  CAST(
  ...
  CONCAT('generated seed order_mt6 ', sequence_numbers.seq) AS comment
FROM (
